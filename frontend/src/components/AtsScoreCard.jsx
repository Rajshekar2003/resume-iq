...
function ScoreCircle({ score }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="text-xs text-slate-500 font-medium uppercase tracking-wide">ATS Score</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <ul className="space-y-1.5">
        {items.map((item, i) => (
          <li key={i} className={`text-sm flex gap-2 items-start ${itemClass}`}>
...
export default function AtsScoreCard({ result }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <h2 className="text-lg font-semibold text-slate-800 mb-5">ATS Analysis</h2>
